...
function clickLabel(label) {
  ...
  _dom.fireEvent.click(label); // clicking the label will trigger a click of the label.control
...
function clickElement(element, previousElement, init) {
  ...
  if (parentLabel == null ? void 0 : parentLabel.control) parentLabel == null ? void 0 : parentLabel.control.focus == null ? void 0 : parentLabel.control.focus();
}
...
function backspace(element) {
  ...
  _dom.fireEvent.keyDown(element, keyboardEventOptions);

  _dom.fireEvent.keyUp(element, keyboardEventOptions);
...
function click(element, init) {
  ...
  if (previouslyFocusedElement) {
    _dom.fireEvent.mouseMove(previouslyFocusedElement);

    _dom.fireEvent.mouseLeave(previouslyFocusedElement);
  }
...
async function type(element, text, {
  ...
  const computedText = element.maxLength > 0 ? text.slice(0, Math.max(element.maxLength - previousText.length, 0)) : text;
  ...
  if (allAtOnce) {
    ...
    for (let index = 0; index < text.length; index++) {
      ...
      const keyCode = char.charCodeAt(0); // eslint-disable-next-line no-await-in-loop
